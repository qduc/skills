...
4. **Resolve disagreements.** When two personas conflict:
   ...
   - **P6 vs. P2/P5:** When P6 says "unnecessary complexity" and P2/P5 says "that machinery prevents a failure," the failure-based finding wins if its scenario passes acceptance rule 2; otherwise P6's finding stands.
...
### P6 — Proportionality Skeptic
- **Charter:** Hunt complexity, abstraction, and scope not justified by a stated requirement. Speculative generality ("we might need"), layers with one caller, configurability nothing configures, plan steps serving no goal, spec requirements serving no stated purpose, tech choices whose operational cost exceeds their benefit at the stated scale. P6 files findings against existing over-engineering in the artifact; it never demands additions — that would be P4's job, and only requirement-backed. For deeper proportionality criteria, see `../proportionality-review/references/criteria.md`.
- **Evidence standard:** Must cite the complexity's location, name the requirement it supposedly serves and show it doesn't (or that no requirement claims it), and state the concrete carrying cost: maintenance burden, cognitive load, added failure modes, or schedule/dollar cost. "This feels over-built" is not a finding; "this abstraction has one implementation, no stated second use, and adds two indirection layers every reader must traverse" is.
- **Stopping condition:** Every major component/abstraction/step has been mapped to a requirement it serves, or filed.

**Persona selection:** All six run by default. Under a Light budget (below), run P1–P3 only and say so. The reference file may mark a persona as low-yield for that artifact type; you may then time-box it rather than skip it.
...
P6 findings cap at **high** — over-engineering wastes and confuses but rarely destroys. Critical is reserved for the case where complexity directly causes a hard-requirement failure (e.g., the added layers blow the latency budget) — and that gets co-filed with P3's arithmetic.
